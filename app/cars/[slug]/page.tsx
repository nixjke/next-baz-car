...
export default function CarDetailPage() {
	...
	useEffect(() => {
		console.log('CarDetailPage - slug:', slug, 'parsed id:', id)
		
		// Если slug еще не загружен, ждем
		if (!slug) {
			console.log('CarDetailPage - slug еще не загружен, ждем...')
			return
		}

		// Если ID не определен, не делаем запрос
		if (!id) {
			console.error('CarDetailPage - ID не указан, slug:', slug)
		...
		const fetchCar = async () => {
			try {
				setLoading(true)
				setError(null)
				console.log('CarDetailPage - Запрос машины с ID:', id)
				const carData = await getCarById(id)
				console.log('CarDetailPage - Получены данные машины:', carData)
				console.log('CarDetailPage - Additional Services:', carData?.additional_services)
				console.log('CarDetailPage - Specifications:', carData?.specifications)
				console.log('CarDetailPage - Description:', carData?.description)
				console.log('CarDetailPage - Features:', carData?.features)
				if (!carData) {
		...
		fetchCar()
	}, [id, slug]) // Убрал params из зависимостей, так как это объект и может вызывать бесконечный цикл
	...
	console.log('CarDetailPage render - loading:', loading, 'error:', error, 'car:', car, 'id:', id, 'slug:', slug)
